...
#define MINIDUMP_FILE_PATH "/data/vendor/minidump"
...
enum ops_type {
	FULL_DUMP_ENABLE = 0,
	FULL_DUMP_DISENABLE = 1,
	MINI_DUMP_ENABLE = 2,
	MINI_DUMP_DISENABLE = 3,
};
...
/*for dump flag*/
#define BIT(x)                          (1 << x)
#define IS_FIRST_BOOT(x)	x & BIT(0) ? 0: 1   /* x & BIT(0) = 1  means not first boot . first boot the value must be 0 .*/
#define FIRST_BOOT              BIT(0)
